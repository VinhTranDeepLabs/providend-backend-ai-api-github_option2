...
def generate_accuracy_test_cases() -> List[LLMTestCase]:
    ...
    accuracy_qa_pairs = [
        ...
        {
            "question": "At what age does Mr. Tan want to retire?",
            "expected_answer": "Mr. Tan wants to retire at age 60, which is in about 15 years."
        },
        {
        ...
        {
            "question": "What did the advisor recommend for CPF contributions?",
            "expected_answer": "The advisor recommended increasing CPF contributions by $500 per month."
        },
        {
            "question": "What life insurance coverage was recommended and why?",
            "expected_answer": "The advisor recommended a term life insurance policy with coverage of about $600,000, which represents 5 years of annual income, to provide income replacement given that Mr. Tan has two young children."
        },
        {
            "question": "What are the next steps after this meeting?",
            "expected_answer": "The advisor will send a detailed proposal by next week including the CPF contribution plan and insurance options, and they will schedule a follow-up meeting."
        }
    ]
...
def generate_grounding_test_cases() -> List[LLMTestCase]:
    ...
    grounding_questions = [
        ...
        "What specific investment products were discussed?",
        "What is the client's risk tolerance?",  # Not explicitly stated - should not hallucinate
        "What insurance recommendations were made?",
        "When will the advisor send the proposal?",
        "What is the client's current annual income?",  # Can be inferred ($120k) but not explicit
        "What was discussed about the client's current bond allocation?"
    ]
...
@pytest.mark.parametrize("test_index", range(6))  # 6 accuracy test cases
...
@pytest.mark.parametrize("test_index", range(8))  # 8 grounding test cases
